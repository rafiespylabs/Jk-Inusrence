        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5>Policy Name : {{$healthpolicy->name}}</h5>
                </div>
                <div class="card-body">
                    <hr>
                    ...
                    <table id="health_policy_renew-datatable" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th>Sl No</th>
                                <th>Start Date</th>
                        ...
                        <tbody>
                            @php $i = 1; @endphp
                            @foreach ($healthpolicy_renews as $renew)
                                <tr id="row{{ $renew->id }}">
                                    <td>{{ $i }}</td>                                            
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              @csrf
                <input type="hidden"  name="healthpolicy_id"   class="form-control" value="{{$healthpolicy->id}}">
                <input type="hidden"  name="policy_cat_id"   class="form-control" value="1">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-4">
                        ...
                        <select  name="payment_mode_id" class="form-control" required>
                            ...
                            <option value="{{$mode->id}}" @if($healthpolicy->payment_mode_id==$mode->id) selected @endif>{{$mode->payment_mode}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-4">
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#health_policy_renew-datatable').DataTable();
        $('#create_renew_form').submit(function(event) 
        {
            event.preventDefault();
            var formData = new FormData($(this)[0]); 
            $.ajax({
                url: "{{route('healthPolicyRenew.store')}}",
                method: "POST",
                data: formData,
                contentType: false, 
                processData: false,
                success: function(response) {
                    if (response.success) 
                    {
                        ...
                        var table = $('#health_policy_renew-datatable').DataTable();
